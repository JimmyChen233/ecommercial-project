...
class UserMsg extends Component<Msg>{
	render(){
		const { create_time, is_from_user, is_read, text, img } = this.props;
		const read = is_read ? '' : 'new'
		const direction = is_from_user ? 'To admin' : 'From admin'
...
export class UserMessage extends Component<Msg> {
	state = {
		messages: [],
		text: '',
		photos: '',
	...
	handleNormalInput = (e: ChangeEvent<HTMLTextAreaElement>) => {
			...
			<div className='message-container'>
				...
				<div>
					<p className='message-mid-title'>History messages:</p>
					{
			        	...
			        	if (msg) {
				            const { create_time, m_id, is_from_user, is_read, user_id, text, img } = msg as Msg;
				            return (
				            	<UserMsg key={index} create_time={create_time} m_id={m_id} is_from_user={is_from_user} is_read={is_read} user_id={user_id} text={text} img={img}/>
				            )
			         	}else {
			        		return null;
			        	}
